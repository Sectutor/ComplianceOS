...
console.log(`  Severities:`, [...new Set(result.findings.map(f => f.severity))]);
console.log(`  Frameworks:`, result.findings[0].frameworkMappings.slice(0, 3));

// 5. Prowler settings export
const { defaultProwlerSettings } = await import('./packages/addons/src/prowler/connector.js');
...
console.log('\n=== ✓ All Phase 2 components pass smoke test ===');
