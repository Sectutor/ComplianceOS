...
export async function getUserById(id: number) {
  ...
  const [raw] = await db.execute(
    sql`SELECT id, open_id, name, email, login_method, last_signed_in, role, created_at,
               deleted_at, max_clients, stripe_customer_id, subscription_status, plan_tier,
               has_seen_tour, access_expires_at
        FROM "users" WHERE id = ${id} LIMIT 1`
  );
  if (!raw) return undefined;
  return {
    id: raw.id, openId: raw.open_id, name: raw.name, email: raw.email,
    loginMethod: raw.login_method, lastSignedIn: raw.last_signed_in, role: raw.role,
    createdAt: raw.created_at, deletedAt: raw.deleted_at, maxClients: raw.max_clients,
    stripeCustomerId: raw.stripe_customer_id, subscriptionStatus: raw.subscription_status,
    planTier: raw.plan_tier, hasSeenTour: raw.has_seen_tour, accessExpiresAt: raw.access_expires_at,
  };
}
