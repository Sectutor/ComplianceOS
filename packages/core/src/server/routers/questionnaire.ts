...
export function createQuestionnaireRouter(t: any, protectedProcedure: any) {
  return t.router({
    /**
...
Output JSON array: [{"questionId": "optional-id", "question": "Question text"}]`;
    ...
    generateAnswers: protectedProcedure
      ...
      .mutation(async ({ input }: any) => {
         ...
         // Process in parallel with concurrency limit (e.g. 3) to not overload DB/LLM
         const batchSize = 3;
         for (let i = 0; i < normalizedQuestions.length; i += batchSize) {
             ...
             const batchPromises = batch.map(async (q: any) => {
                 try {
                     ...
                     console.error('[GenerateAnswers] Error:', e);
                     return {
                         questionId: q.questionId,
                         question: q.question,
            ...
            try {
                ...
                await IndexingService.indexDocument(
                    ...
                    content,
                    {
                        ...
                        question: q.question,
                        answer: q.answer,
                        origin: 'questionnaire_completion'
                    }
                );
                indexedCount++;
            } catch (e) {
                console.error(`Failed to index answer ${q.id}`, e);
            }
        }));
