/**
 * OWASP SAMM v2 TRPC Router
 * 
 * This router implements proper SAMM v2 methodology with stream-based assessments.
 * Each of the 15 practices has 2 streams (A and B), resulting in 30 assessment streams.
 */
...
export const sammV2Router = router({
    // ============================================================================
    ...
    getPracticeQuestions: clientProcedure
        ...
        .query(async ({ input }) => {
            ...
            // Group by stream
            const streamA = questions.filter(q => q.streamId === 'A');
            const streamB = questions.filter(q => q.streamId === 'B');
    ...
    calculatePracticeScore: clientProcedure
        ...
        .query(async ({ input }) => {
            ...
            const streamA = streams.find(s => s.streamId === 'A');
            const streamB = streams.find(s => s.streamId === 'B');
    ...
    calculateOverallScore: clientProcedure
        ...
        .query(async ({ input }) => {
            ...
            practices.forEach(practice => {
                const streamA = assessments.find(a => a.practiceId === practice.practiceId && a.streamId === 'A');
                const streamB = assessments.find(a => a.practiceId === practice.practiceId && a.streamId === 'B');
    ...
    generateImprovementPlan: clientProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const gaps = assessments.filter(a => a.maturityLevel < a.targetLevel);
