...
export type AuditAction = 'create' | 'update' | 'delete' | 'publish' | 'export' | 'sync' | 'restore' | 'import' | 'acknowledge_compliance' | 'update_security_setup' | 'confirm_asset_receipt' | 'assign' | 'withdraw' | 'consent_created' | 'dsar_status_updated' | 'dpia_template_created' | 'request_review' | 'approve_policy' | 'reject_policy' | 'comment' | 'trigger';
export type EntityType = 'policy' | 'control' | 'client' | 'user' | 'evidence' | 'mapping' | 'risk' | 'treatment' | 'asset' | 'threat' | 'vulnerability' | 'bcp_plan' | 'roadmap' | 'implementation_plan' | 'stakeholder' | 'dev_project' | 'threat_model' | 'employee_acknowledgment' | 'employee_security' | 'employee_asset' | 'training_module' | 'training_assignment' | 'project' | 'task' | 'compliance_mapping' | 'consent' | 'dsar_request' | 'dpia_template' | 'data_flow' | 'security_test' | 'work_item' | 'autopilot_run';
