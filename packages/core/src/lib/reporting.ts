...
export async function generateCustomProfessionalReport(clientId: number, options: {
    ...
    const drawSectionHeader = (title: string) => {
        ...
        doc.fillColor('white').fontSize(24).font('Helvetica-Bold').text(title.toUpperCase(), 50, 40);
        doc.moveDown(4);
        doc.fillColor(slateGray).fontSize(10).font('Helvetica');
    };

    // Helper: Call AI for section content
    const getAIContent = async (section: string, data: any): Promise<string> => {
        try {
            const response = await llmService.generate({
                systemPrompt: `You are a Senior Cyber Security & Compliance Advisor. Write a professional, executive-level section for a compliance intelligence report. Focus on strategic implications and risk posture. Use professional British/American English.`,
            ...
            console.error(`AI Generation failed for section ${section}:`, error);
            return "Professional analysis temporarily unavailable. Raw data indicates ongoing compliance activities and strategic mapping for this domain.";
        }
    };

    // Helper: Render text
    const renderContent = (text: string) => {
        doc.fillColor(slateGray).fontSize(11).font('Helvetica').text(text, {
            align: 'justify',
            lineGap: 4
        });
        doc.moveDown(2);
    ...
    doc.fillColor('white').fontSize(42).font('Helvetica-Bold').text('COMPLIANCE', 50, 200);
    doc.fontSize(28).font('Helvetica-Bold').text('INTELLIGENCE REPORT', 50, 245);
    doc.rect(50, 290, 400, 2).fill('white');

    doc.fontSize(16).font('Helvetica').text(options.title, 50, 320);
    doc.moveDown();
    doc.fontSize(12).text(`Client Context: ${client.name}`, 50);
    doc.text(`Industry: ${client.industry || 'General Business'}`, 50);

    doc.fontSize(10).text(`Generated by ComplianceOS Professional AI Analytics`, 50, doc.page.height - 100);
    doc.text(new Date().toLocaleDateString(undefined, { dateStyle: 'long' }), 50, doc.page.height - 85);
    ...
    if (options.sections.includes('executive_summary')) {
        ...
        const score = await getClientComplianceScore(clientId);
        const stats = await getClientStats(clientId);
        const summary = await getAIContent('Executive Overview', { client, score, stats });
        renderContent(summary);
    }

    // 3. Gap Analysis
    if (options.sections.includes('gap_analysis')) {
        drawSectionHeader('Gap Analysis & Maturity');
        const score = await getClientComplianceScore(clientId);
        const controls_data = await getClientControls(clientId);
        const summary = await getAIContent('Maturity Assessment', { score, total_controls: controls_data.length });
        renderContent(summary);
    }

    // 4. Risks
    if (options.sections.includes('risks')) {
        drawSectionHeader('Risk Portfolio');
        ...
        const summary = await getAIContent('Strategic Risk Posture', { risks });
        renderContent(summary);
    }

    // 5. Controls
    if (options.sections.includes('controls')) {
        drawSectionHeader('Control Verification');
        const controls_data = await getClientControls(clientId);
        const summary = await getAIContent('Control Verification & Efficacy', {
            controls: controls_data.slice(0, 15).map(c => ({ id: c.controlId, name: c.name, status: c.clientControl?.status }))
        });
        renderContent(summary);
    }
    ...
    if (options.sections.includes('bcp')) {
        drawSectionHeader('Business Continuity');
        const { bcpProjects } = await import('../schema');
        const projects = await dbConn.select().from(bcpProjects).where(eq(bcpProjects.clientId, clientId)).limit(5);
        const summary = await getAIContent('Resilience & Recovery Operations', { projects });
        renderContent(summary);
    }

    // 7. BIA
    if (options.sections.includes('bia')) {
        drawSectionHeader('Business Impact Analysis');
        const { businessImpactAnalyses } = await import('../schema');
        const bias = await dbConn.select().from(businessImpactAnalyses).where(eq(businessImpactAnalyses.clientId, clientId)).limit(5);
        const summary = await getAIContent('Impact Criticality Analysis', { bias });
        renderContent(summary);
    }

    // 8. Assets
    if (options.sections.includes('assets')) {
        drawSectionHeader('Asset Inventory');
        const { assets } = await import('../schema');
        const assetList = await dbConn.select().from(assets).where(eq(assets.clientId, clientId)).limit(10);
        const summary = await getAIContent('Organizational Asset Landscape', { assets: assetList });
        renderContent(summary);
    }

    // 9. Vendors
    if (options.sections.includes('vendors')) {
        drawSectionHeader('Vendor Risk');
        ...
        const summary = await getAIContent('Third-Party Risk Profile', { vendors: vendorList });
        renderContent(summary);
    }

    // 10. Incidents
    if (options.sections.includes('incidents')) {
        drawSectionHeader('Incident & Event Log');
        ...
        const summary = await getAIContent('Security Incident Analysis', { incidents: incidentList });
        renderContent(summary);
    }

    // 11. Vulnerabilities
    if (options.sections.includes('vulnerabilities')) {
        drawSectionHeader('Vulnerability Management');
        ...
        const summary = await getAIContent('Technical Exposure & Patch Management', { vulnerabilities: vulnList });
        renderContent(summary);
    }

    // 12. Audit
    if (options.sections.includes('audit')) {
        drawSectionHeader('Internal Audit');
        ...
        const summary = await getAIContent('Audit Observations', { findings });
        renderContent(summary);
    }
