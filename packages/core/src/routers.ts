...
const checkClientAccess = t.middleware(async ({ ctx, next, rawInput }) => {
  ...
  console.log(`[Middleware] checkClientAccess. User: ${ctx.user.id}, ClientId in Input: ${clientId}`);

  if (!clientId) {
    console.error(`[Middleware] No clientId found in input. Blocking request.`);
  ...
  try {
    ...
    if (membership.length === 0) {
      console.error(`[Middleware] Access Denied. User ${ctx.user.id} is not a member of Client ${clientId}`);
      throw new TRPCError({ code: 'FORBIDDEN', message: 'No access to this client workspace' });
    }

    console.log(`[Middleware] Access Granted. Role: ${membership[0].role}`);
    return next({ ctx: { ...ctx, clientId, clientRole: membership[0].role } });
  } catch (err) {
    console.error(`[Middleware] Error checking client access:`, err);
    if (err instanceof TRPCError) throw err;
    throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Authorization check failed" });
  }
});
...
export const appRouter = router({
  ...
  vendors: router({
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          try {
            ...
            console.error("Failed to index vendor:", e);
          }

          // AI VRM Agent: Trigger Discovery in Background
          if (newVendor.website || newVendor.name) {
            (async () => {
              try {
                const { vrmAgent } = await import('./lib/ai/vrm-agent');
                const url = await vrmAgent.discoverTrustCenter(newVendor.name, newVendor.website || undefined);
                if (url) {
                  await vrmAgent.analyzeTrustCenter(newVendor.id, url);
                }
              } catch (e) {
                console.error("Background trust center discovery failed:", e);
              }
            })();
          }
      ...
      .mutation(async ({ input }) => {
        ...
        if (updated) {
          try {
            ...
            console.error("Failed to index updated vendor:", e);
          }
        }
    ...
    delete: adminProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input }) => {
        // Fetch to get clientId? Or assumed? adminProcedure input implies we might need clientId.
        // But deleteVendor takes only ID.
        // We need clientId for deleteDocumentIndex.
        // We must fetch vendor first.
        const db = await getDb();
        const vendor = await db.query.vendors.findFirst({ where: eq(schema.vendors.id, input.id) });
        ...
        if (vendor) {
          try {
            const { IndexingService } = await import('./lib/advisor/indexing');
            await IndexingService.deleteDocumentIndex(vendor.clientId, 'vendor', input.id.toString());
          } catch (e) {
            console.error("Failed to delete vendor index:", e);
          }
        }
        return true;
      }),
    ...
    scan: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const mockVendors = [
          { name: "Slack", description: "Collaboration Platform", category: "SaaS", source: "SSO", criticality: "Medium", website: "https://slack.com" },
          { name: "Zoom", description: "Video Conferencing", category: "SaaS", source: "SSO", criticality: "Medium", website: "https://zoom.us" },
          { name: "Figma", description: "Design Software", category: "SaaS", source: "SSO", criticality: "Low", website: "https://figma.com" },
          { name: "AWS", description: "Cloud Infrastructure", category: "IaaS", source: "SSO", criticality: "High", website: "https://aws.amazon.com" },
          { name: "Stripe", description: "Payment Processing", category: "Service", source: "Finance", criticality: "High", website: "https://stripe.com" }
        ];

        // Randomly pick 1-3 vendors to "discover"
        const shuffled = mockVendors.sort(() => 0.5 - Math.random());
    ...
    discoverTrustCenter: adminProcedure
      .input(z.object({ vendorId: z.number() }))
      .mutation(async ({ input }) => {
        console.log(`[TRPC] vendors.discoverTrustCenter started for Vendor ID: ${input.vendorId}`);
        const db = await getDb();
        const vendor = await db.query.vendors.findFirst({ where: eq(schema.vendors.id, input.vendorId) });

        if (!vendor) {
          console.error(`[TRPC] vendors.discoverTrustCenter: Vendor NOT FOUND for ID: ${input.vendorId}`);
          throw new TRPCError({ code: "NOT_FOUND", message: "Vendor not found" });
        }

        console.log(`[TRPC] vendors.discoverTrustCenter: Found vendor "${vendor.name}". Triggering vrmAgent...`);

        const { vrmAgent } = await import('./lib/ai/vrm-agent');
        const url = await vrmAgent.discoverTrustCenter(vendor.name, vendor.website || undefined);

        console.log(`[TRPC] vendors.discoverTrustCenter: Discovery returned URL: ${url}`);

        if (url) {
          await db.update(schema.vendors).set({ trustCenterUrl: url }).where(eq(schema.vendors.id, input.vendorId));
          // Automatically trigger analysis IN THE BACKGROUND so the mutation returns quickly
          // This prevents HTTP timeouts during slow LLM analysis
          (async () => {
            try {
              console.log(`[TRPC] Background Analysis: Starting for Vendor ${vendor.name} at ${url}`);
              await vrmAgent.analyzeTrustCenter(vendor.id, url);
              console.log(`[TRPC] Background Analysis: COMPLETED for Vendor ${vendor.name}`);
            } catch (e) {
              console.error(`[TRPC] Background Analysis: FAILED for Vendor ${vendor.name}:`, e);
            }
          })();
        }
    ...
    analyzeTrustCenter: adminProcedure
      .input(z.object({ vendorId: z.number(), url: z.string().url() }))
      .mutation(async ({ input }) => {
        const { vrmAgent } = await import('./lib/ai/vrm-agent');
        const analysis = await vrmAgent.analyzeTrustCenter(input.vendorId, input.url);
        return analysis;
      }),

    runRiskScan: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // 1. Scan Vulnerabilities (Real NVD Search)
        console.log(`[RiskScan] Searching NVD for vendor: ${vendor.name}`);
        const nvdResults = await threatIntel.searchNvdByKeyword(vendor.name, 20);
        const vulnerabilities = nvdResults?.vulnerabilities || [];
        console.log(`[RiskScan] NVD returned ${vulnerabilities.length} vulnerabilities`);
        ...
        // 2. Scan Breaches (Simulated for now)
        console.log(`[RiskScan] simulating breach search for: ${vendor.name}`);
        const foundBreaches = await threatIntel.simulateBreachSearch(vendor.name, vendor.website || undefined);
        let breachScore = 0;

        // Calculate breach impact on score
        ...
        // Save Scan
        const [scan] = await db.insert(schema.vendorScans).values({
          clientId: input.clientId,
          vendorId: input.vendorId,
        ...
        // Save Breaches linked to this vendor (and implicitly scan via date if needed, though table links to vendor)
        if (foundBreaches.length > 0) {
          for (const breach of foundBreaches) {
            await db.insert(schema.vendorBreaches).values({
              vendorId: input.vendorId,
              title: breach.title,
              ...
              status: 'Active'
            }).onConflictDoNothing(); // Simple dedup
          }
        }

        // Save CVEs linked to this scan
        if (cveMatches.length > 0) {
          for (const match of cveMatches.slice(0, 10)) {
            await db.insert(schema.vendorCveMatches).values({
              ...match,
              scanId: scan.id // Link CVE to this specific scan
            }).onConflictDoNothing();
          }
        }
    ...
    getScanVulnerabilities: adminProcedure
      .input(z.object({ scanId: z.number() }))
      .query(async ({ input }) => {
        const db = await getDb();
        return db.query.vendorCveMatches.findMany({
          where: eq(schema.vendorCveMatches.scanId, input.scanId),
          orderBy: [desc(schema.vendorCveMatches.discoveredAt)],
        });
      }),

    getLatestScan: adminProcedure
      .input(z.object({ vendorId: z.number() }))
      .query(async ({ input }) => {
        const db = await getDb();

        // Get ALL scans for this vendor, sorted by date descending
        const scans = await db.query.vendorScans.findMany({
          where: eq(schema.vendorScans.vendorId, input.vendorId),
          orderBy: [desc(schema.vendorScans.scanDate)]
        });
        ...
        const vulnerabilities = await db.query.vendorCveMatches.findMany({
          where: eq(schema.vendorCveMatches.scanId, latestScan.id),
          orderBy: [desc(schema.vendorCveMatches.discoveredAt)],
        });

        const breaches = await db.query.vendorBreaches.findMany({
          where: eq(schema.vendorBreaches.vendorId, input.vendorId),
          orderBy: [desc(schema.vendorBreaches.breachDate)]
        });
    ...
    createRemediationTask: adminProcedure
      .input(z.object({
        clientId: z.number(),
        vendorId: z.number(),
        cveId: z.string(),
        cveDescription: z.string().optional(),
      ...
      .mutation(async ({ input }) => {
        ...
        const [task] = await db.insert(schema.projectTasks).values({
          clientId: input.clientId,
          title: `[CVE Remediation] ${input.cveId} - ${vendor?.name || 'Vendor'}`,
          description: `**CVE ID:** ${input.cveId}\n**CVSS Score:** ${input.cvssScore || 'N/A'}\n**Vendor:** ${vendor?.name || 'Unknown'}\n\n**Description:**\n${input.cveDescription || 'No description'}\n\n**Notes:**\n${input.notes || 'No additional notes'}`,
          status: 'todo',
          priority: input.priority || 'high',
          dueDate: input.dueDate ? new Date(input.dueDate) : null,
          tags: ['cve-remediation', input.cveId, vendor?.name || 'vendor'].filter(Boolean) as string[],
        }).returning();
