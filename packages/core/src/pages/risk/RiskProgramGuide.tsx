...
import {
    ...
    AlertTriangle, Clock, DollarSign, Compass,
...
export default function RiskProgramGuide(props?: { id?: string | number; clientId?: string | number }) {
    ...
    const [returnToStartHere, setReturnToStartHere] = useState<string | null>(() => {
        ...
        try {
            ...
            if (returnTo && returnTo.includes('/start-here')) {
                ...
                sessionStorage.setItem(`risk_start_here_origin_${clientId}`, payload);
                return returnTo;
            }
            ...
            const stored = sessionStorage.getItem(`risk_start_here_origin_${clientId}`);
            if (stored) {
            ...
            if (startHereStored) {
                ...
                if (parsed?.url && parsed.url.includes('/start-here') && (Date.now() - (parsed.timestamp || 0)) < 2 * 60 * 60 * 1000) {
                    sessionStorage.setItem(`risk_start_here_origin_${clientId}`, JSON.stringify({
                        url: parsed.url,
                        timestamp: Date.now()
                    }));
                    return parsed.url;
    ...
    const handleReturnToStartHere = () => {
        try {
            sessionStorage.removeItem(`risk_start_here_origin_${clientId}`);
                    ...
                    <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 shrink-0 w-full lg:w-80 space-y-3">
                            ...
                            <Button
                                ...
                                className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs mt-2 rounded-lg h-8 gap-1.5 shadow"
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="border border-indigo-200 dark:border-indigo-900/50 rounded-xl p-5 bg-indigo-50/50 dark:bg-indigo-950/20 space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Badge className="bg-indigo-100 text-indigo-800 text-[10px] font-bold">Standardized</Badge>
                                </div>
